...
contract('Proposal test with cancellation on COMPLETED', async accounts => {
    ...
    it('transition to CLOSE from COMPLETED', async() => {
        let arbiterReward = await proposalInstance.arbiterDaiReward.call();
        let contractorReward = await proposalInstance.contractorDaiReward.call();
        ...
        let currentCustomerTokenBalance = await token.balanceOf(CUSTOMER_1);
        let contractorCurrentBalance = await token.balanceOf(CONTRACTOR_1);

        assert.equal(customerTokenBalanceBeforeClose.toNumber() + arbiterReward.toNumber(), currentCustomerTokenBalance.toNumber())
        assert.equal(contractorCurrentBalance.toNumber(), contractorReward.toNumber())
    })
})